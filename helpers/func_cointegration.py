...
# Calculate Z-Score
def calculate_zscore_old(spread, window):
...
def calculate_zscore(spread, window):
    ...
    mean = np.convolve(spread, np.ones(window), 'valid') / window
    std = np.array([np.std(spread[i:i+window]) for i in range(len(spread) - window + 1)])
    x = spread[window-1:]
    zscore = (x - mean) / std
    return zscore
